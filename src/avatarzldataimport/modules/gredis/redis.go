...
func Setup() error {
	RedisConn = &redis.Pool{
		...
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", config.RedisConfig.Host+":"+strconv.FormatInt(int64(config.RedisConfig.Port), 10))
			if err != nil {
				return nil, err
			}
			...
			c.Do("SELECT", config.RedisConfig.Database)
			return c, err
		},
...
func HSet(key string, field string, data interface{}, time int) (bool, error) {
	conn := RedisConn.Get()
	defer conn.Close()
	...
	reply, err := redis.Bool(conn.Do("HSET", key, field, value))
	conn.Do("EXPIRE", key, time)

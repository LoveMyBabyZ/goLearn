...
var Product = make([]THosProduct, 0)

type THosProduct struct {
	AddTime                string  `json:"add_time" xorm:"not null default 'CURRENT_TIMESTAMP' comment('添加时间') TIMESTAMP"`
	...
	LastModify             string  `json:"last_modify" xorm:"not null default 'CURRENT_TIMESTAMP' comment('更新时间') TIMESTAMP"`
	...
	ProductType            int     `json:"product_type" xorm:"not null default '1046' comment('产品类型') VARCHAR(10)"`
	...
	RefPrice               float64 `json:"ref_price" xorm:"not null default 0.00 comment('入库含税参考价') DECIMAL(30,2)"`
	Remark                 string  `json:"remark" xorm:"not null default '' comment('备注／注意事项') VARCHAR(2000)"`
	SellPrice              float64 `json:"sell_price" xorm:"not null default 0.00 comment('销售单价') DECIMAL(30,2)"`
	...
	Tax                    float64 `json:"tax" xorm:"not null default 0.00 comment('税额') DECIMAL(30,2)"`
	TaxRate                float64 `json:"tax_rate" xorm:"not null default 0.00 comment('税率') DECIMAL(30,2)"`
	Traits                 string  `json:"traits" xorm:"not null default '' comment('性状') VARCHAR(255)"`
	UntaxedPrice           float64 `json:"untaxed_price" xorm:"not null default 0.00 comment('未税单价') DECIMAL(30,2)"`
	WarehouseCode          string  `json:"warehouse_code" xorm:"default '' comment('大仓编码') VARCHAR(50)"`
}

// 获取产品名称
func GetProductName() (arr map[string]int, err error) {
	err = db.DB.Cols("product_name","bar_code","product_code").Where("company_id = ?", 2).Find(&Product)
	if err != nil {
		fmt.Println(err)
	}
...
// 获取产品条形码
func GetBarCode() (arr map[string]int, err error) {
	arr = make(map[string]int)
	for _, v := range Product {
		arr[v.BarCode] = 1
	}
...
// 获取产品条形码
func GetProductCode() (arr map[string]int, err error) {
	arr = make(map[string]int)
	for _, v := range Product {
		arr[v.BarCode] = 1
	}
...
func (self *THosProduct) InsertTest() {
	self.ProductName = "赵德勇测试"
	self.ProductCode = "CXXXX001"
	self.AddTime = time.Now().Format("2006-01-02 15:04:05")
	self.LastModify = time.Now().Format("2006-01-02 15:04:05")
	session := db.DB.NewSession()
	affected, err := session.Insert(self)
	fmt.Println(affected, err)
}

func InsertProduct(products []*THosProduct) {
	var a int
	le := len(products)
	if le > 100 {
		a = le/100 + 1
	...
	for i := 0; i < a; i++ {
		...
		insertProducts := products[i*100 : max]
		//fmt.Println(insertProducts[0].AddTime)
		//fmt.Println(len(insertProducts))
		affected, err := db.DB.Insert(&insertProducts)
		db.DB.ShowSQL(true)
		fmt.Println(affected, err)
	}

...
type THosSupplier struct {
	...
	Status                     int       `json:"status" xorm:"not null default 1 comment('状态') TINYINT(2)"`
...
func GetSupplierArr() (arr map[string]string, err error) {
	Supplier := make([]THosSupplier, 0)
	err = db.DB.Cols("supplier_name", "supplier_code").Where("company_id = ? AND is_producer = ?", 2, 1).Find(&Supplier)
	if err != nil {
		fmt.Println(err)
	}
	arr = make(map[string]string)
	for _, v := range Supplier {
		arr[v.SupplierName] = v.SupplierCode
	}
	return
}

...
type MdPackageDetail struct {
	...
	ItemCode       string    `json:"item_code" xorm:"default '0' comment('产品编码') VARCHAR(50)"`
	...
	OrgId          int       `json:"org_id" xorm:"not null default 0 comment('机构ID') index(idx_org_packid) index(idx_org_proid) INT(11)"`
	PackageId      int       `json:"package_id" xorm:"not null comment('套餐关联ID') index(idx_org_packid) INT(11)"`

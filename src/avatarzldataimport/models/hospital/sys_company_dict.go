...
var Dict = make([]SysCompanyDict, 0)

func init() {
	err := db.HospitalDb.Cols("name", "code","parent_code").Where("brand_id = ? AND status = ? ", 13, 1).Find(&Dict)
	//fmt.Println(Dict)
	if err != nil {
		fmt.Println(err)
	}
}
func GetDict(code int) (arr map[string]int, err error) {
	//Dict := make([]SysCompanyDict, 0)
	//fmt.Println(code)

	arr = make(map[string]int)
	for _, v := range Dict {
		//fmt.Println(v.ParentCode)
